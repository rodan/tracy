...
This tracking device allows fine-grained timing customization. It can either preserve battery life for more than a month when sparse measurements are being made or it can record positional data every 10 seconds if needed. Either behaviour can be set by the user via SMS commands, without the need to recompile the firmware.
